...
def build_targets(pred_corners, target, num_keypoints, num_anchors, num_classes, nH, nW, noobject_scale, object_scale, sil_thresh, seen):
    ...
    num_labels = 2 * num_keypoints + 1 # +2 for width, height and +1 for class within label files
    nAnchors = nA*nH*nW
    nPixels  = nH*nW
    for b in range(nB):
...
def build_targets_anchors(pred_corners, target, num_keypoints, anchors, num_anchors, num_classes, nH, nW, noobject_scale, object_scale, sil_thresh, seen):
    nB = target.size(0)
    nA = num_anchors
    nC = num_classes
...
class RegionLoss(nn.Module):
    def __init__(self, num_keypoints=8, num_classes=1, anchors=[], num_anchors=3, pretrain_num_epochs=0):
        ...
        self.coord_scale         = 1  #1
        self.noobject_scale      = 0.1  #1
        self.object_scale        = 5  #5
        self.class_scale         = 1
        self.thresh              = 0.5
        self.seen                = 0

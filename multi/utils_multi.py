...
def get_multi_region_boxes(output, conf_thresh, num_classes, num_keypoints, anchors, num_anchors, only_objectness=0, validation=False):
            ...
            for cx in range(w):
                for i in range(num_anchors):
                    ...
                    if conf > conf_thresh:
                        ...
                        boxes.append(box) 
        if (len(boxes) == 0):
            # If there are no boxes or no boxes for the corresponding class, append None
...
def get_multi_region_boxes_valid(output, conf_thresh, num_classes, num_keypoints, anchors, num_anchors, correspondingclass, only_objectness=1, validation=False):
